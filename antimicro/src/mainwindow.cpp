    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->uacPushButton->setVisible(false);
#endif
    this->arduinoConnector = new ArduinoConnector(joysticks, settings, this);
...
void MainWindow::fillButtons(QMap<SDL_JoystickID, InputDevice *> *joysticks)
{
    ...
    while (iter.hasNext())
    {
        ...
        // FIXXXME michi
        connect(tabwidget, SIGNAL(joystickConfigChanged(int)), this, SLOT(resendJoystickConfigChanged(int)));
        if (showTrayIcon)
        {
...
void MainWindow::autoprofileLoad(AutoProfileInfo *info)
{
  if( info != NULL ) {
    ...
    {
    #endif
    qDebug() << "autoProfileLoad::: info:" << info->getProfileLocation() << "isDefault:" << info->isCurrentDefault() << "isActive:" << info->isActive();
    for (int i = 0; i < ui->tabWidget->count(); i++)
    {
        JoyTabWidget *widget = static_cast<JoyTabWidget*>(ui->tabWidget->widget(i));
        if (widget)
        {
            qDebug() << "widget curr conf name" << widget->getCurrentConfigName() << "curr conf index" << widget->getCurrentConfigIndex() << "joy" << widget->getJoystick()->getName();
            if (info->getGUID() == "all")
            {
            ...
            else if (info->getGUID() == widget->getJoystick()->getStringIdentifier())
            {
                if (info->getProfileLocation().isEmpty())
                {
                    qDebug() << "set Current Config (0)";
                    widget->setCurrentConfig(0);

                }
                else
                {
                    qDebug() << "calling loadConfig with" << info->getProfileLocation();
                    widget->loadConfigFile(info->getProfileLocation());
...
void MainWindow::resendJoystickConfigChanged(int stick)
{
    emit autoProfileLoaded();
}
